...
def WithOptimizationLearningRateDecayMomentum(minibatchsize, Nsteps):
    ...
    train_df = pd.DataFrame(
        {"file": os.listdir("C:/Users/Levin_user/Desktop/Kaggle/train/train")})
    ...
    # 1st Layer
        ...
        optimizer = Optimizer_SGD(
            learning_rate=0.0002, decay=0.001, momentum=0.1)
    ...
    # calling loss function
        ...
        # data too massiv to run it all with slow script(?)
        for batch in range(0, MaxBatches):

            for iteration in range(Nsteps):
                # passing data through layer
                x = []
                for image in range(0, minibatchsize):
                    img = [io.imread('C:/Users/Levin_user/Desktop/Kaggle/train/resized/' +
                                     test['file'].iloc[image+batch*minibatchsize])]
                    x.append(img)
            ...
            # backward passes
                ...
                # lets see if the conv1 is a problem
